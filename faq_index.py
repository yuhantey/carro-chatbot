...
def build_faq_index(chunks: list[str]):
    """Build or load FAQ index"""
    model, embeddings, index, cached_chunks = load_model_artifacts()
    
    if model is not None and cached_chunks == chunks:
    ...
    save_model_artifacts(model, embeddings, index, chunks)
